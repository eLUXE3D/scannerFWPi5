...
def get_interfaces(interface="wlan0"):
    ...
    """
    iwList=call_iwlist(interface).decode()
    iwListSplit=iwList.split('\n')
    return get_parsed_cells(iwListSplit)



def getWifiStrengthAndQuality():
    try:
        networks = get_interfaces(interface='wlan0')
        network=networks[0]
        #print(networks)
        strength=network['Signal Level']
        quality=network['Quality']
    except Exception as e:
        print('wifi strength error: ',e)
        return None, None
    
    print('Wifi strength, quality:',strength, quality)
    return strength, quality
    
#getWifiStrengthAndQuality()